...
SYMPY_VALIDATORS = [
    RegexValidator(
        r"^[a-zA-Z0-9\*\/_\s\+\-\(\)]+$",
        message="Please use allowed characters only"
        " (chars, numbers, +, -, * , /, ( and ))",
        flags=MULTILINE,
    ),
]
...
def non_commutative_sympify(string: str):
    """Evaluate sympy string in non-commutative fashion.
...
class SympyField(TextField):
    ...
    def value_to_string(self, obj: Expr) -> str:
        """Serialize object by calling `get_prep_value`."""
    ...
    def from_db_value(
        ...
        """Convert db entry (string or None) to expression or None."""
        if value is None:
            return value
        try:
    ...
    def to_python(self, value: Union[None, str, Expr]) -> Expr:
        """Convert value (string, None, or expression) to sympy expression."""
        if value is None:
            return value
        elif isinstance(value, Expr):
            return value
